...
final class NoWhitespaceBeforeCommaInArrayFixer extends AbstractFixer
{
    ...
    {
        return $tokens->isAnyTokenKindsFound([T_ARRAY, CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN]);
    }
    ...
    protected function applyFix(\SplFileInfo $file, Tokens $tokens)
    {
        for ($index = $tokens->count() - 1; $index >= 0; --$index) {
            if ($tokens[$index]->isGivenKind([T_ARRAY, CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN])) {
    ...
    {
        if ($tokens[$index]->isGivenKind(CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN)) {
            $startIndex = $index;
            $endIndex = $tokens->findBlockEnd(Tokens::BLOCK_TYPE_ARRAY_TRANSACTPRO_BRACE, $startIndex);
        } else {
    ...
    private function skipNonArrayElements($index, Tokens $tokens)
    {
        ...
        if ($tokens[$index]->equals(')')) {
            ...
            if (!$tokens[$startIndex]->isGivenKind([T_ARRAY, CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN])) {
                return $startIndex;
            }
        }
