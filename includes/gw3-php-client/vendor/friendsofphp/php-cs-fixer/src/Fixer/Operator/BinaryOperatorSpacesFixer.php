...
',
    ...
    private function injectAlignmentPlaceholders(Tokens $tokens, $startAt, $endAt, $tokenContent)
    {
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if ($token->equals('[')) {
                $index = $tokens->findBlockEnd(Tokens::BLOCK_TYPE_INDEX_TRANSACTPRO_BRACE, $index);

                continue;
            }

            if ($token->isGivenKind(CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN)) {
                $index = $tokens->findBlockEnd(Tokens::BLOCK_TYPE_ARRAY_TRANSACTPRO_BRACE, $index);
    ...
    private function injectAlignmentPlaceholdersForArrow(Tokens $tokens, $startAt, $endAt)
    {
        for ($index = $startAt; $index < $endAt; ++$index) {
            ...
            if ($token->isGivenKind(CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN)) {
                $from = $index;
                $until = $tokens->findBlockEnd(Tokens::BLOCK_TYPE_ARRAY_TRANSACTPRO_BRACE, $from);
                $index = $until;
            ...
            if ($token->equals(',')) {
                for ($i = $index; $i < $endAt - 1; ++$i) {
                    ...
                    if ($tokens[$i + 1]->isGivenKind([T_ARRAY, CT::T_ARRAY_TRANSACTPRO_BRACE_OPEN])) {
