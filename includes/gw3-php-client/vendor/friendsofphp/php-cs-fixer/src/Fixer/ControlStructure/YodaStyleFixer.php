...
',
    ...
    {
        for ($i = $index; $i <= $end; ++$i) {
            if ($tokens[$i]->isGivenKind([T_LIST, CT::T_DESTRUCTURING_TRANSACTPRO_BRACE_OPEN, CT::T_DESTRUCTURING_TRANSACTPRO_BRACE_CLOSE])) {
                return true;
            }
        }
    ...
    private function isVariable(Tokens $tokens, $index, $end)
    {
        ...
        while ($index <= $end) {
            ...
            if ($current->isGivenKind([T_LIST, CT::T_DESTRUCTURING_TRANSACTPRO_BRACE_OPEN, CT::T_DESTRUCTURING_TRANSACTPRO_BRACE_CLOSE])) {
                return false;
            }
            ...
            if ($current->isGivenKind($expectString ? T_STRING : T_VARIABLE) && $next->equals('[')) {
                $index = $tokens->findBlockEnd(Tokens::BLOCK_TYPE_INDEX_TRANSACTPRO_BRACE, $index + 1);
                if ($index === $end) {
                    return true;
                }
